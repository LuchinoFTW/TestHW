exports.handler = async (event) => {
  ...
  try {
    ...
    const categoryText = {
      ...
      orale:      `Ha dedicato ${values[8]} punti all'abilità nel sesso orale. Questa scelta rivela il desiderio di ricevere piacere pieno e dedicato, di essere messa al centro dell'attenzione senza riserve — un bisogno di essere completamente al centro del desiderio dell'altro.`,
      penetrativo:`Ha dedicato ${values[9]} punti all'esperienza penetrativa completa. Questo parla del desiderio di vivere l'esperienza nella sua totalità, senza mezze misure — è la voglia di sentirsi completamente presenti e appagata in ogni sua dimensione.`,
    };

    const q3text = q3score >= 5
      ? `Ha dedicato ${q3score} punti all'attrazione verso qualcuno che già conosce. Questo è un segnale molto significativo: ha intuito che la componente più intensa di questa dinamica non è fisica ma emotiva. Sentirsi attratta da qualcuno con cui c'è già una connessione reale rende tutto più vivo e coinvolgente. Un modo delicato per esplorare questa dimensione con il partner potrebbe essere condividere apertamente con lui su quali persone che entrambi conoscono ha mai fantasticato — non come confessione ma come apertura di una conversazione onesta sul desiderio.`
      : '';

    const top2text = top2.map(p => categoryText[p.id] || '').filter(Boolean).join(' ');

    const prompt = `Sei una psicologa specializzata in relazioni consensualmente non monogame. Una donna ha completato un test distribuendo 50 punti tra 10 caratteristiche del suo terzo ideale.
...
${top2text ? `POI ANALIZZA: ${top2text} Per ciascuna di queste caratteristiche, rifletti su come questa preferenza si colleghi al desiderio di riaccendere l'intensità emotiva nella coppia — non attraverso conflitto ma attraverso la consapevolezza che il desiderio dell'altro si rinnova quando si sente coinvolto, presente, necessario. Esplora come queste dinamiche, vissute con consapevolezza e comunicazione, possano aiutare entrambi i partner a riscoprire l'attrazione che il tempo tende ad attenuare.` : ''}
